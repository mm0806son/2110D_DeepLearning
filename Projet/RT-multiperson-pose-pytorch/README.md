...
- Le notebook BlurFacesWithOpenpose.ipynb qui comprend l'ensemble des codes et des commentaires. Une fois dézippé, 2 actions sont à effectuer avant de pouvoir exécuter le notebook :
1) Modifier la variable colab_path dans la première cellule en passant le lien vers le répertoire courant où se situe le fichier BlurFacesWithOpenpose.ipynb
2) S'assurer que le dossier RT-multiperson-pose-pytorch contenant toutes les ressources se situe dans ce même répertoire.

NB: le fichier *.avi qui représente la vidéo obtenue en sortie de la partie Blur with a Circle, peut être regénérée en exécutant le notebook avec l'option TEST_MODE désactivée. Cette option est située dans "Run the experiments" de la partie Blur with a Circle
Le fichier de sortie se situera alors dans le répertoire RT-multiperson-pose-pytorch, sous le nom filename.avi

- Les fichiers de données sont dans dataTpPose Keraal. Il comprend un fichier readme.txt qui detaille les formats des données
